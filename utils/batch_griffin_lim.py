...
def _transform_ops():
    return [
        ...
        'remove_nodes(op=Identity, op=CheckNumeric)',
        'fold_batch_norms',
...
def _get_node_name(tensors):
    if isinstance(tensors, list):
        return [tensor.name.split(':') for tensor in tensors]
    else:
        return tensors.name.split(':')
...
with tf.Session(graph=tf.Graph()) as sess:
    ...
    with tf.gfile.FastGFile('gf_model.pb', mode='rb') as fin:
        fin.write(constant_graph.SerializeToString())
    ...
    try:
        ...
        tf.train.write_graph(constant_graph_def, logdir='root/chenfeng', name='final1/optimized_gf_with_length.pb',
                             as_text=False)
