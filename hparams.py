...
hparams = tf.contrib.training.HParams(
    ...
    cut_synthesis_audio=False,  # cut audio when synthesis in case of it cannot stop
    ...
    upsample_type='1D',
    # Type of the upsampling deconvolution. Can be ('1D' or '2D'). 1D spans all frequency bands for each frame while 2D spans "freq_axis_kernel_size" bands at a time
    ...
    # Only used for 2D upsampling. This is the number of requency bands that are spanned at a time for each frame.
    ...
    # Whether to Apply Saliman & Kingma Weight Normalization (reparametrization) technique. (NEEDS VERIFICATION)
